...
self.addEventListener('message', function(e) {
    ...
    try{
        if(e.data.encrypt){
            fileData = CryptoJS.AES.encrypt(e.data.file, e.data.password);
        } else {
            fileData = CryptoJS.AES.decrypt(e.data.file, e.data.password).toString(CryptoJS.enc.Utf8);
            fileData=fileData.replace(/\n/g,"");
        }
        self.postMessage({'text' : fileData.toString()});
    } catch(e) {
